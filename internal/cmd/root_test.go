...
func TestNew_UsesGhCommandNameWhenRunningUnderGh(t *testing.T) {
	...
	t.Setenv("GH_EXTENSION", "users")
	executableName = func() string { return "gh-users" }
...
func TestRun_PrintsTSVOutput(t *testing.T) {
...
func TestRun_PrintsEmptyOutputWhenNoMatches(t *testing.T) {
	streams, _, stdout, _ := iostreams.Test()
	mock := &mockService{
		response: &ghclient.QueryEnvelope{
			Data: ghclient.QueryResponse{
